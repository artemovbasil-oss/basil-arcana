export interface SofiaPersonaProfile {
  displayName: string;
  handle: string;
...
export function buildSofiaPersonaProfile(displayName: string, handle: string): SofiaPersonaProfile {
  return {
    displayName,
    handle,
    publicIdentity:
      `${displayName} (${handle}) is an explicitly AI-authored Telegram persona inside Basil Arcana. ` +
...
export function buildSofiaSystemPrompt(displayName: string, handle: string): string {
