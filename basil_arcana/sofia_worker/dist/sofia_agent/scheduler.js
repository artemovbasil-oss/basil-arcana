...
async function ensureDefaultCommunityTargets(config) {
    const existing = await (0, db_1.listSofiaSearchTargets)(false);
    ...
    for (const target of defaults) {
        ...
        await (0, db_1.createSofiaSearchTarget)({
            label: target.label,
            query: target.query,
...
async function ensureManualCommunityTargets(config) {
    if (!config.communityModeEnabled) {
        return;
    }
    const existing = await (0, db_1.listSofiaSearchTargets)(false);
    const existingByHandle = new Map(existing
        .map((target) => [normalizeHandle(target.targetChat), target])
    ...
    for (const target of existing) {
        ...
        if (category !== "social" && target.enabled) {
            await (0, db_1.setSofiaSearchTargetEnabled)(target.id, false, {
                disabledReason: "social_only_mode",
                disabledAt: new Date().toISOString(),
            });
        }
    }
    const manualTargets = MANUAL_SOCIAL_COMMUNITIES.map((url) => ({ url, category: "social" }));
    const manualHandles = new Set(manualTargets
    ...
    for (const item of manualTargets) {
        ...
        if (existingTarget) {
            const disabledReason = typeof existingTarget.metadata?.disabledReason === "string"
                ? existingTarget.metadata.disabledReason
                : null;
            const shouldKeepDisabled = disabledReason === "access_lost";
            if (shouldKeepDisabled) {
                await (0, db_1.resetSofiaSearchTarget)(existingTarget.id, false, {
            ...
            await (0, db_1.resetSofiaSearchTarget)(existingTarget.id, true, {
                source: "manual_allowlist",
                category: item.category,
                workflow: "review_first",
                noPromoToday: true,
...
function extractTelegramHandleFromUrl(url) {
    if (!url) {
        return null;
    }
...
function isInviteOnlyTelegramUrl(url) {
    return /t\.me\/\+/i.test(url);
}
function isCommunityAccessLossError(error) {
    const message = error instanceof Error ? error.message : String(error);
    return /CHANNEL_PRIVATE|CHAT_ADMIN_REQUIRED|CHAT_FORBIDDEN|CHANNEL_INVALID|USER_BANNED_IN_CHANNEL|PEER_ID_INVALID/i.test(message);
}
function buildDiscoveryExclusionHandles(config, approverHandle) {
...
function candidateStateKey() {
    return "community_candidate_approval";
}
function communityDraftApprovalStateKey() {
    return "community_draft_approval";
}
function communityPriority(target) {
    const category = typeof target.metadata?.category === "string" ? target.metadata.category : "";
    if (category === "social") {
        return 0;
    }
...
function isSocialCategory(target) {
    return target.metadata?.category === "social";
}
function socialHandleSet() {
    return new Set(MANUAL_SOCIAL_COMMUNITIES.map((url) => extractTelegramHandleFromUrl(url)).filter((value) => Boolean(value)));
}
function socialRhythmMode(now) {
...
function isCommunityDraftApproveCommand(text) {
    const normalized = text.trim().toLowerCase();
    return normalized === "ok" || normalized === "send" || normalized === "approve";
}
function isCommunityDraftCancelCommand(text) {
    const normalized = text.trim().toLowerCase();
    return normalized === "cancel" || normalized === "skip" || normalized === "reject";
}
function isRejectMessage(text) {
    const normalized = text.toLowerCase();
    return (normalized.includes("ищи другие") ||
...
function extractApprovalIds(text, maxId) {
    const normalized = text.toLowerCase();
...
function buildCandidateApprovalMessage(candidates) {
    const lines = [
...
function buildRelocationSuggestionMessage(candidates) {
    const lines = [
...
async function maybeProcessCommunityApprovals(config) {
    const approverHandle = normalizeHandle(config.outreachReportChat);
    if (!approverHandle) {
        return;
    }
    const pending = await (0, db_1.getSofiaRuntimeState)(candidateStateKey());
    const items = Array.isArray(pending?.items) ? pending?.items : [];
    if (!items.length) {
        return;
    }
    const processedIds = new Set(Array.isArray(pending?.processedMessageIds)
        ? (pending?.processedMessageIds).map((value) => String(value))
        : []);
    const inbound = await (0, db_1.listRecentInboundSofiaMessages)(50);
    ...
    const existingTargets = await (0, db_1.listSofiaSearchTargets)(false);
    for (const message of approverMessages) {
        ...
        if (isRejectMessage(text)) {
            ...
            await (0, db_1.setSofiaRuntimeState)(candidateStateKey(), {
                items: [],
        ...
        console.info(`[${new Date().toISOString()}] community shortlist approved by ${approverHandle}: ids=${approvedIds.join(",")}`);
        ...
        for (const approvedId of approvedIds) {
            ...
            if (existingTarget) {
                if (!existingTarget.enabled) {
                    await (0, db_1.setSofiaSearchTargetEnabled)(existingTarget.id, true, {
...
async function maybeProcessCommunityDraftApprovals(config) {
    const approverHandle = normalizeHandle(config.outreachReportChat);
    if (!approverHandle) {
        return;
    }
    const state = (await (0, db_1.getSofiaRuntimeState)(communityDraftApprovalStateKey())) ?? {};
    const currentTaskId = typeof state.taskId === "number"
        ? state.taskId
    ...
    const processedIds = new Set(Array.isArray(state.processedMessageIds)
    ...
    for (const message of approverMessages) {
        ...
        if (isCommunityDraftApproveCommand(text)) {
            await (0, db_1.markSofiaAgentTaskApproved)(taskId);
            await (0, db_1.setSofiaRuntimeState)(communityDraftApprovalStateKey(), {
                taskId: null,
                processedMessageIds: Array.from(processedIds),
                lastDecision: "approved",
                updatedAt: new Date().toISOString(),
        ...
        if (isCommunityDraftCancelCommand(text)) {
            await (0, db_1.markSofiaAgentTaskFailed)(taskId, "Cancelled by approver");
            await (0, db_1.setSofiaRuntimeState)(communityDraftApprovalStateKey(), {
                taskId: null,
                processedMessageIds: Array.from(processedIds),
...
async function maybeRequestCommunityDraftApproval(config) {
    const approverHandle = normalizeHandle(config.outreachReportChat);
    if (!approverHandle) {
        return;
    }
    const state = (await (0, db_1.getSofiaRuntimeState)(communityDraftApprovalStateKey())) ?? {};
    const existingTaskId = typeof state.taskId === "number"
        ? state.taskId
        : typeof state.taskId === "string"
            ? Number(state.taskId)
            : null;
    ...
    const draftReadyTasks = await (0, db_1.listSofiaAgentTasksByStatus)("draft_ready", 50);
    ...
    const draft = await (0, db_1.getLatestDraftForTask)(nextTask.id);
    if (!draft) {
        return;
    }
...
async function maybeDiscoverCommunityCandidates(config) {
    const approverHandle = normalizeHandle(config.outreachReportChat);
    ...
    if (existingPending.length) {
        console.info(`[${new Date().toISOString()}] community shortlist pending approval: count=${existingPending.length}`);
        return;
    }
    const targets = await (0, db_1.listSofiaSearchTargets)(true);
    const discoveryTargets = targets.filter((target) => target.targetChat === null && (target.metadata?.mode ?? "") === "discovery");
    if (!discoveryTargets.length) {
        return;
    }
    const approvedChats = new Set(targets
    ...
    for (const target of discoveryTargets) {
        ...
        console.info(`[${new Date().toISOString()}] discovery target checked: label=${target.label} query="${target.query}" matches=${matches.length}`);
        for (const match of matches) {
            ...
            if (textLower.includes("tarot_arkana_bot") ||
            ...
            const resolvedCandidate = await (0, mtproto_1.resolveCommunityTarget)(config, normalizedTarget);
    ...
    console.info(`[${new Date().toISOString()}] community shortlist sent to ${approverHandle}: count=${items.length}`);
    await (0, db_1.setSofiaRuntimeState)(candidateStateKey(), {
        items,
        processedMessageIds: [],
...
async function maybeSuggestRelocationCommunities(config) {
    const approverHandle = normalizeHandle(config.outreachReportChat);
    if (!approverHandle || !config.communityModeEnabled) {
        return;
    }
    const stateKey = "relocation_discovery_shortlist";
    const state = (await (0, db_1.getSofiaRuntimeState)(stateKey)) ?? {};
    const lastSentAt = typeof state.lastSentAt === "string" ? Date.parse(state.lastSentAt) : 0;
    const now = Date.now();
    if (Number.isFinite(lastSentAt) && now - lastSentAt < 12 * 60 * 60 * 1000) {
        return;
    }
    const existingTargets = await (0, db_1.listSofiaSearchTargets)(false);
    const excludedHandles = new Set(existingTargets
        .map((target) => normalizeHandle(target.targetChat))
        .filter((value) => Boolean(value)));
    ...
    for (const query of RELOCATION_DISCOVERY_QUERIES) {
        ...
        for (const match of matches) {
            ...
            try {
                resolvedCandidate = await (0, mtproto_1.resolveCommunityTarget)(config, normalizedTarget);
            }
            catch {
                continue;
            }
...
async function syncTgstatCommunityTargets(config) {
    ...
    const candidates = await (0, tgstat_1.fetchTgstatCommunityCandidates)(config);
    let activated = 0;
    for (const candidate of candidates) {
        const existing = byHandle.get(normalizeHandle(candidate.targetChat));
        if (existing) {
            if (!existing.enabled) {
                await (0, db_1.setSofiaSearchTargetEnabled)(existing.id, true, {
...
function isLikelyCommunityQuestionText(text) {
    const normalized = text.trim().toLowerCase();
    if (!normalized)
        return false;
...
function isMeaningfulCommunityText(text) {
    const normalized = text.replace(/\s+/g, " ").trim();
    if (!normalized) {
        return false;
    }
...
function isLikelyBroadcastOrAdminPost(match) {
...
function localHourParts(date, timeZone) {
...
async function maybeScheduleOwnedChannelPosts(config) {
    ...
    const candidates = [
        ...
        },
        {
            ...
            notes: "Write the post in Russian. Use @ostorozhno_novosti as a source of light celebrity or lifestyle notes only. No politics, no crime, no tragedy, no negativity. Do not repost. Rewrite in Sofia's own words and add a short original astro/tarot angle.",
        },
    ];
    let created = 0;
    for (const candidate of candidates) {
    ...
    if (lifeNoteReady) {
        await (0, db_1.createSofiaAgentTask)({
            ...
            payload: {
                ...
                channelHandle: config.ownedChannelHandle,
                recommendBotUrl: config.recommendBotUrl,
                editorialNotes: "Write a small personal-feeling channel note in Russian about travel, a recent purchase, a street detail, a cafe, a book, or another lived-in moment. Keep it warm and believable, but do not claim real-world facts that require verification.",
                language: "ru",
            },
        });
...
function buildOutreachReportMessage(items) {
    if (items.length === 0) {
...
async function maybeSendOutreachHourlyReport(config) {
    ...
    const items = await (0, db_1.listSentSofiaOutreachSince)(sinceMs);
...
async function runSofiaSearchSchedulerOnce(config) {
    ...
    const dueTargets = (await (0, db_1.listDueSofiaSearchTargets)()).sort((a, b) => {
    ...
    const recentSocialOutreach = new Set((await (0, db_1.listSentSofiaOutreachSince)(now - 30 * 60 * 1000))
        .map((item) => normalizeHandle(item.targetChat))
        .filter((value) => Boolean(value)));
    const sentSocialLastHour = (await (0, db_1.listSentSofiaOutreachSince)(now - 60 * 60 * 1000)).filter((item) => {
        const handle = normalizeHandle(item.targetChat);
        return handle ? socialHandles.has(handle) : false;
    }).length;
    const sentSocialLastTwoHours = (await (0, db_1.listSentSofiaOutreachSince)(now - 2 * 60 * 60 * 1000)).filter((item) => {
        const handle = normalizeHandle(item.targetChat);
        return handle ? socialHandles.has(handle) : false;
    }).length;
    const socialQueuedThisCycle = new Set();
    for (const target of dueTargets) {
        const mode = target.metadata?.mode ?? (target.targetChat ? "active" : "discovery");
        ...
        try {
            await (0, mtproto_1.withSofiaTelegramClient)(config, async (client) => {
                const resolvedTarget = target.targetChat
                    ? await (0, mtproto_1.resolveCommunityTargetWithClient)(client, target.targetChat)
                    : null;
                if (resolvedTarget && !resolvedTarget.isWritableCommunity) {
                    console.info(`[${new Date().toISOString()}] community target skipped as non-writable: requested=${resolvedTarget.requestedTarget} kind=${resolvedTarget.targetKind}`);
                    await (0, db_1.setSofiaSearchTargetEnabled)(target.id, false, {
                ...
                if (resolvedTarget?.usedLinkedDiscussion) {
                    console.info(`[${new Date().toISOString()}] community target rerouted to discussion: requested=${resolvedTarget.requestedTarget} effective=${resolvedTarget.effectiveTarget}`);
                }
                ...
                const matches = await (0, mtproto_1.searchTelegramMessagesWithClient)(client, {
                    query: target.query,
                ...
                const candidateMatches = (questionMatches.length ? questionMatches : eligibleMatches.slice(0, 1)).slice(0, isSocialCategory(target) ? 1 : questionMatches.length ? questionMatches.length : 1);
                ...
                const normalizedTargetChat = normalizeHandle(resolvedTarget?.effectiveTarget ??
                    target.targetChat ??
                ...
                if (isSocialCategory(target) && normalizedTargetChat) {
                    if (recentSocialOutreach.has(normalizedTargetChat) || socialQueuedThisCycle.has(normalizedTargetChat)) {
                        console.info(`[${new Date().toISOString()}] social target rate-limited: target=${normalizedTargetChat}`);
                        return;
                    }
                }
                for (const match of candidateMatches) {
                    ...
                    await (0, db_1.createSofiaAgentTask)({
                        ...
                        targetChat: resolvedTarget?.effectiveTarget ??
                            target.targetChat ??
                        ...
                        payload: {
                            ...
                            surroundingMessages: await (0, mtproto_1.fetchTelegramMessageWindowWithClient)(client, {
                                targetChat: resolvedTarget?.effectiveTarget ??
                                    target.targetChat ??
            ...
            if (isCommunityAccessLossError(error)) {
                console.info(`[${new Date().toISOString()}] community target disabled after access loss: target=${target.targetChat ?? "GLOBAL"} error=${errorMessage}`);
                await (0, db_1.setSofiaSearchTargetEnabled)(target.id, false, {
            ...
            console.info(`[${new Date().toISOString()}] community target disabled after resolve failure: target=${target.targetChat ?? "GLOBAL"} error=${errorMessage}`);
            await (0, db_1.setSofiaSearchTargetEnabled)(target.id, false, {
