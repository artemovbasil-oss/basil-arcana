...
function applySeoMeta(path) {
  let ogImageUrl = "https://app.basilarcana.com/og-image.svg";
  const defaultMeta = {
    title: "Astronautica",
  ...
  const sharedNatalCode = isSharedNatalPage ? String(path.replace("/shared-natal/", "")).trim() : "";
  if (sharedNatalCode) {
    ogImageUrl = `https://app.basilarcana.com/og/shared-natal/${encodeURIComponent(sharedNatalCode)}.svg`;
  }
  const meta = article
    ? {
